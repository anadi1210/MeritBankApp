...
public class AccountHolderController {
	...
	@GetMapping("/AccountHolders/{name}")
	@ResponseStatus(HttpStatus.OK)
	public AccountHolder getAccountHolder(@PathVariable (name = "name") String name) {
		//AccountHolder ah = accountRepo.findByUsername(name);
		return accountHolderService.getAccountHolder(name);//.get(); 
	}
	
	@DeleteMapping("/AccountHolders/{name}")
	public String deleteAccountHolder(@PathVariable (name = "name") String name) {
		long id = getAccountHolderId(name);
		if(accountHolderService.hasCheckingAccount(id) || accountHolderService.hasSavingsAccount(id) 
				|| accountHolderService.hasCDAccount(id)) {
			return "Account holder can not be deleted. Unclosed Accounts Exists";
		}
